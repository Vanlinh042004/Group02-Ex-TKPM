...
import { exportStudentsToCSV, exportStudentsToJSON } from "../Home/ExportStudents"; // ✅ Import đúng hàm
...
function Home() {
  ...
  const handleImport = async (file) => {
    try {
      const result = await ImportStudents(file); // Đảm bảo importStudents xử lý file đúng
      setStudents(result);
  ...
  <div className="col-md-4 d-flex" key={index}>
    <div className="student-card align-self-stretch p-4 mb-4">
      ...
      <p><b>Khoa:</b> {student.faculty?.name}</p>
      ...
      {/* Địa chỉ thường trú */}
      <p><b>Địa chỉ thường trú:</b> {`${student.permanentAddress?.streetAddress}, ${student.permanentAddress?.district}, ${student.permanentAddress?.city}`}</p>

      {/* Chứng minh nhân dân */}
      <p><b>CMND:</b> {student.identityDocument?.number} - <b>Nơi cấp:</b> {student.identityDocument?.issuePlace}</p>
