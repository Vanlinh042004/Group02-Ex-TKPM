...
describe('PhoneNumberConfigService', () => {
  ...
  describe('getAllPhoneNumberConfigs', () => {
    it('should return all phone number configurations', async () => {
      // Arrange
      const mockConfigs = [
        { country: 'Việt Nam', countryCode: '+84', regex: '^(0|\\+84)[0-9]{9}$' },
    ...
    it('should throw error when there is a problem getting configs', async () => {
      ...
      // Act & Assert
      await expect(phoneNumberConfigService.getAllPhoneNumberConfigs())
  ...
  describe('getPhoneNumberConfig', () => {
    it('should return the phone number configuration for a specific country', async () => {
      // Arrange
      const mockConfig = { country: 'Việt Nam', countryCode: '+84', regex: '^(0|\\+84)[0-9]{9}$' };
      (PhoneNumberConfig.findOne as jest.Mock).mockResolvedValue(mockConfig);

      // Act
      const result = await phoneNumberConfigService.getPhoneNumberConfig('Việt Nam');
    ...
    it('should throw error when country is not provided', async () => {
      // Act & Assert
      await expect(phoneNumberConfigService.getPhoneNumberConfig(''))
    ...
    it('should throw error when configuration not found', async () => {
      ...
      // Act & Assert
      await expect(phoneNumberConfigService.getPhoneNumberConfig('Unknown'))
  ...
  describe('addPhoneNumberConfig', () => {
    it('should add a new phone number configuration', async () => {
      ...
      // Mock constructor
      (PhoneNumberConfig as unknown as jest.Mock).mockImplementation(() => savedConfig);
      // Mock findOne to return null (no existing config)
      (PhoneNumberConfig.findOne as jest.Mock).mockResolvedValue(null);

      // Act
      const result = await phoneNumberConfigService.addPhoneNumberConfig(newConfig);

      // Assert
      expect(PhoneNumberConfig.findOne).toHaveBeenCalledWith({ country: newConfig.country });
    ...
    it('should throw error when country is not provided', async () => {
      ...
      // Act & Assert
      await expect(phoneNumberConfigService.addPhoneNumberConfig(invalidConfig as any))
        .rejects.toThrow('Country is required');
      expect(PhoneNumberConfig.findOne).not.toHaveBeenCalled();
    });
    ...
    it('should throw error when regex is invalid', async () => {
      ...
      // Act & Assert
      await expect(phoneNumberConfigService.addPhoneNumberConfig(invalidConfig))
    ...
    it('should throw error when country already exists', async () => {
      ...
      (PhoneNumberConfig.findOne as jest.Mock).mockResolvedValue(existingConfig);

      // Act & Assert
      await expect(phoneNumberConfigService.addPhoneNumberConfig(existingConfig))
        .rejects.toThrow('Phone number configuration for Germany already exists');
      expect(PhoneNumberConfig.findOne).toHaveBeenCalledWith({ country: existingConfig.country });
  ...
  describe('updatePhoneNumberConfig', () => {
    it('should update an existing phone number configuration', async () => {
      ...
      (PhoneNumberConfig.findOneAndUpdate as jest.Mock).mockResolvedValue(updatedConfig);

      // Act
      const result = await phoneNumberConfigService.updatePhoneNumberConfig(country, updateData);
    ...
    it('should throw error when regex is invalid', async () => {
      ...
      // Act & Assert
      await expect(phoneNumberConfigService.updatePhoneNumberConfig(country, updateData))
        .rejects.toThrow('Invalid regex pattern');
  ...
  describe('deletePhoneNumberConfig', () => {
    it('should delete a phone number configuration', async () => {
      ...
      (PhoneNumberConfig.findOneAndDelete as jest.Mock).mockResolvedValue(deletedConfig);

      // Act
      const result = await phoneNumberConfigService.deletePhoneNumberConfig(country);

      // Assert
      expect(PhoneNumberConfig.findOneAndDelete).toHaveBeenCalledWith({ country });
    ...
    it('should throw error when configuration not found', async () => {
      ...
      // Act & Assert
      await expect(phoneNumberConfigService.deletePhoneNumberConfig(country))
        .rejects.toThrow('No phone number configuration found for Unknown');
      expect(PhoneNumberConfig.findOneAndDelete).toHaveBeenCalledWith({ country });
    });
  });
