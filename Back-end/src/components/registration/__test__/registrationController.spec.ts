...
describe("RegistrationController", () => {
  ...
  it("should register course successfully", async () => {
    ...
    (RegistrationService.registerCourse as jest.Mock).mockResolvedValue(
      mockData,
    );

    await RegistrationController.registerCourse(req, res);

    expect(RegistrationService.registerCourse).toHaveBeenCalledWith(
      "SV001",
      "CSC10001-11",
    );
  ...
  it("should return error when registration fails", async () => {
    ...
    const res = mockRes();
    (RegistrationService.registerCourse as jest.Mock).mockRejectedValue(
  ...
  it("should fetch all registrations", async () => {
    ...
    (RegistrationService.getAllRegistrations as jest.Mock).mockResolvedValue(
      mockData,
    );
  ...
  it("should fetch all students from class", async () => {
    ...
    (
      RegistrationService.getAllStudentsFromClass as jest.Mock
    ).mockResolvedValue(mockData);

    await RegistrationController.getAllStudentsFromClass(req, res);

    expect(RegistrationService.getAllStudentsFromClass).toHaveBeenCalledWith(
      "CSC10001-11",
    );
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith({
  ...
  it("should generate transcript successfully", async () => {
    ...
    (RegistrationService.generateTranscript as jest.Mock).mockResolvedValue(
      mockData,
    );

    await RegistrationController.generateTranscript(req, res);

    expect(RegistrationService.generateTranscript).toHaveBeenCalledWith(
      "SV001",
    );
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith({
