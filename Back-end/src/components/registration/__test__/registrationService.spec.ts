...
describe('RegistrationService - registerCourse', () => {
  ...
  it('should pass when prerequisites are completed', async () => {
    ...
    const mockSave = jest.fn().mockResolvedValue({ _id: 'regId', student: student._id });
    (Registration as any).mockImplementation(() => ({ save: mockSave }));

    const result = await registrationService.registerCourse('student123', 'class123');
...
describe('RegistrationService - cancelRegistration', () => {
  it('should cancel registration if active', async () => {
    ...
    const result = await registrationService.cancelRegistration('regId', 'reason');
  ...
  it('should throw if registration not found', async () => {
    ...
    await expect(registrationService.cancelRegistration('regId', 'reason')).rejects.toThrow('Đăng ký không tồn tại');
  });

  it('should throw if registration is not active', async () => {
    const registration = { _id: 'regId', status: 'cancelled', class: 'classId' };
    (Registration.findById as jest.Mock).mockReturnValue({
      populate: jest.fn().mockResolvedValue(registration),
    });

    await expect(registrationService.cancelRegistration('regId', 'reason')).rejects.toThrow(
      'Chỉ được hủy đăng ký cho các đăng ký đang hoạt động'
    );
  });
});
...
describe('RegistrationService - updateGrade', () => {
  it('should throw if grade invalid', async () => {
    await expect(registrationService.updateGrade('regId', 11)).rejects.toThrow('Điểm số không hợp lệ (0-10)');
  });

  it('should throw if registration not found', async () => {
    (Registration.findById as jest.Mock).mockResolvedValue(null);
    await expect(registrationService.updateGrade('regId', 9)).rejects.toThrow('Đăng ký không tồn tại');
  });

  it('should throw if registration is not active', async () => {
    (Registration.findById as jest.Mock).mockResolvedValue({ status: 'cancelled' });
  ...
  it('should update grade successfully', async () => {
    (Registration.findById as jest.Mock).mockResolvedValue({ _id: 'regId', status: 'active' });
    (Registration.findByIdAndUpdate as jest.Mock).mockResolvedValue({ _id: 'regId', grade: 9 });
...
describe('RegistrationService - getAllStudentsFromClass', () => {
  it('should return students of a class', async () => {
    ...
    const result = await registrationService.getAllStudentsFromClass('class123');
    expect(result).toEqual([{ _id: 'reg1' }]);
  });

  it('should throw if class not found', async () => {
    (Class.findOne as jest.Mock).mockResolvedValue(null);
    await expect(registrationService.getAllStudentsFromClass('class123')).rejects.toThrow('Lớp học không tồn tại');
  });
});
...
describe('RegistrationService - generateTranscript', () => {
 ...
 it('should throw if student not found in generateTranscript', async () => {
  ...
  await expect(registrationService.generateTranscript('abc')).rejects.toThrow('Sinh viên không tồn tại');
});
